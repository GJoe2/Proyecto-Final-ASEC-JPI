...
np.savetxt('ENTREGABLE\lista_filtro.txt',lstf)
#==================================================================
...
#ITERACIONES
contador=0
df6 = pd.DataFrame(columns=['#Niveles','Nmodel','a(m)','b(m)','h(m)','t(m)','Lm(m)','V(kN)','Δmax(‰)','VolConc(m3)']) #Para guardar los datos
for zz in range(nnz):
    nz = int(Nz[zz])
    Lmopt = 0
    for nmodel in range(nlstf) :
        ...
        Lm = lstf[nmodel,3]
        h = lstf[nmodel,4]
        Volumen = lstf[nmodel,5]*nz

        print('Modelo N° %.0f para edificio de %.0f niveles'%(nmodel+1,nz))
...
print(df6.round(4))
df6.to_csv('ENTREGABLE\Modelos_analizados.csv')
